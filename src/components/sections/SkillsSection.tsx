...
const SkillsSection: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDataAndSetupThreeJS = async () => {
      try {
        setLoading(true);
        const { data, error: fetchError } = await supabase
          .from('heroskills')
          .select('name, icon');
        if (fetchError) throw new Error(`Failed to fetch skills data: ${fetchError.message}`);
        if (!data) throw new Error('No skills data found');

        setSkills(data);

        // Initialize Three.js only after data is successfully fetched
        if (canvasRef.current) {
          ...
          const starMaterial = new THREE.ShaderMaterial({
            ...
            vertexShader: `
              ...
              void main() {
                ...
                float scale = size * (1.9 + 0.1 * sin(uTime + time));
            ...
            fragmentShader: `
              ...
              varying vec3 vColor;
              void main() {
                gl_FragColor = vec4(vColor, 0.6) * texture2D(uTexture, gl_PointCoord);
                float intensity = 1.0 - length(gl_PointCoord - 0.5) * 2.0;
                gl_FragColor.rgb += 0.1 * intensity;
              }
            `,
            transparent: true,
            vertexColors: true,
          ...
          const animateParticles = () => {
            ...
            starMaterial.uniforms.uTime.value += 0.05;
    ...
    fetchDataAndSetupThreeJS();
  }, []);

  useEffect(() => {
    // GSAP Animation for skill cards
    if (loading || error || !skillsSectionRef.current) return;
